...
function spanNum (n){
  if (typeof n === 'string') {
    return new VNode('span', {}, null, n);
  } else {
    return new VNode('span', { key: n }, null, n.toString());
  }
}
...
describe("vnode patch: children", function (){
  let vnode0;
  beforeEach(function (){
    vnode0 = new VNode("p", {}, [ createTextVNode("hello world") ]);
    patch(null, vnode0);
  })

  it("should append children", function (){
    const vnode1 = new VNode("p", null, [ 1 ].map(spanNum));
  ...
  it("should move to left and leaves hold", function (){
    const vnode1 = new VNode('p', {}, [ 1, 4, 6 ].map(spanNum));
    const vnode2 = new VNode('p', {}, [ 4, 5 ].map(spanNum));
    let elm      = patch(vnode0, vnode1);
    expect(elm.children.length).toBe(3);
    elm = patch(vnode1, vnode2);
  ...
  it("should reserve element", function (){
    const vnode1 = new VNode('p', {}, [ 1, 2, 3, 4, 5, 6 ].map(spanNum));
    const vnode2 = new VNode('p', {}, [ 6, 5, 4, 3, 2, 1 ].map(spanNum));
    let elm      = patch(vnode0, vnode1);
  ...
  it("should remove elements with update children without key", function (){
    const vnode1 = new VNode('p', {}, [
      new VNode('span', {}, null, 'one'),
      new VNode('span', {}, null, 'two'),
      new VNode('span', {}, null, 'three')
    ])
    const vnode2 = new VNode('p', {}, [
      new VNode('span', {}, null, 'one'),
      new VNode('span', {}, null, 'three')
  ...
  it("should remove a single text node with updating children without key", function (){
    const vnode1 = new VNode('p', {}, null, 'one');
    const vnode2 = new VNode('p', {});
    let elm      = patch(vnode0, vnode1);
  ...
  it("should remove a single text node when children are updated", function (){
    const vnode1 = new VNode('div', {}, null, 'one');
    const vnode2 = new VNode('div', {}, [
      new VNode('div', {}, null, 'two'),
      new VNode('span', {}, null, 'three')
    ])
    let elm      = patch(vnode0, vnode1);
    expect(elm.textContent).toBe('one');
  ...
  it("should handle children with the same key but with different tag", function (){
    const vnode1 = new VNode('div', {}, [
      new VNode('div', { key: 1 }, null, 'one'),
      new VNode('div', { key: 2 }, null, 'two'),
      new VNode('div', { key: 3 }, null, 'three'),
      new VNode('div', { key: 4 }, null, 'four'),
    ])
    const vnode2 = new VNode('div', {}, [
      new VNode('div', { key: 4 }, null, 'four'),
      new VNode('span', { key: 3 }, null, 'three'),
      new VNode('span', { key: 2 }, null, 'two'),
      new VNode('div', { key: 1 }, null, 'one')
    ])
    let elm      = patch(vnode0, vnode1);
